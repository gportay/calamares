...
PartitionLayout::PartitionEntry::PartitionEntry( const QString& mountPoint,
    ...
    , partMaxSize( maxSize )
{
    PartUtils::findFS( fileSystem, &partFileSystem );
}
...
void
PartitionLayout::init( const QString& defaultFileSystemType )
{
    bool ok;
    ...
    for ( const auto& r : m_list )
    {
        ...
        if ( !addEntry( { CalamaresUtils::getString( pentry, "name" ),
                          ...
                          CalamaresUtils::getString( pentry, "filesystem", defaultFileSystemType ),
    ...
    {
        addEntry( { QString( "/" ), defaultFileSystemType, QString( "100%" ) } );
    }
}
