...
ChoicePage::init( PartitionCoreModule* core )
{
    ...
    connect( m_reuseHomeCheckBox, &QCheckBox::stateChanged,
             this, &ChoicePage::onHomeComboBoxStateChanged );
...
void
ChoicePage::onLeave()
ChoicePage::onHomeComboBoxStateChanged()
{
...
ChoicePage::doReplaceSelectedPartition( const QModelIndex& current )
{
    ...
    cDebug() << __func__ << "@" << __FILE__ << "@" << __LINE__ << "doReuseHomePartition" << doReuseHomePartition;
    cDebug() << __func__ << "@" << __FILE__ << "@" << __LINE__ << "homePartitionPath" << homePartitionPath
    ...
    [ this, current ]( QString* homePartitionPath, bool doReuseHomePartition )
    {
        ...
        {
            cDebug() << __FILE__ << "@" << __func__ << "@" << __LINE__;
            ...
            cDebug() << __FILE__ << "@" << __func__ << "@" << __LINE__ << "layoutApply...";
        ...
        else
        {
            ...
            if ( selectedPartition )
            {
                ...
                {
                        cDebug() << __FILE__ << "@" << __func__ << "@" << __LINE__ << "osproberEntry.path" << osproberEntry.path;
                        cDebug() << __FILE__ << "@" << __func__ << "@" << __LINE__ << "partPath" << partPath; if ( osproberEntry.path == partPath )
                        *homePartitionPath = osproberEntry.homePath;
                }
		if (homePartitionPath)
                    cDebug() << __func__ << "@" << __FILE__ << "@" << __LINE__ << "homePartitionPath" << *homePartitionPath;
...
ChoicePage::updateActionChoicePreview( ChoicePage::InstallChoice choice )
{
    ...
    CalamaresUtils::unmarginLayout( layout );
    layout->setSpacing( 6 ); // TODO spacing?
    ...
    qWarning() << "...";
    if ( m_choice == Alongside || m_choice == Replace )
    {
    	qWarning() << "yeah!";
...
ChoicePage::setupHomePartitionSelector()
{
    ...
    if ( partitions.count() == 0 ) //should never happen
    {
        m_homePartitionLabel->setText(
                    tr( "An home partition cannot be found anywhere "
                        "on this system. Please go back and use manual "
                        "partitioning to set up %1." )
                    .arg( Calamares::Branding::instance()->shortProductName() ) );
        updateNextEnabled();
    }
    else if ( partitions.count() == 1 ) //probably most usual situation
    {
        m_homePartitionLabel->setText(
                    tr( "The home partition at %1 will be used for %2" )
    ...
    else
    {
        ...
        for ( int i = 0; i < partitions.count(); ++i )
        {
            ...
            // We pick an home on the currently selected device, if possible
...
ChoicePage::setupActions()
{
          ...
          it != PartitionIterator::end( currentDevice ); ++it )
    {
        ...
        if ( (*it)->isMounted() )
        {
            cDebug() << Logger::SubEntry << "mounted" << it;
